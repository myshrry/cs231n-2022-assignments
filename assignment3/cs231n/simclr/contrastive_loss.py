...
def sim(z_i, z_j):
    ...
    norm_dot_product = (z_i.T @ z_j) / (torch.linalg.norm(z_i) * torch.linalg.norm(z_j))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss += - torch.log(torch.exp(sim(z_k, z_k_N) / tau) / d1) - torch.log(torch.exp(sim(z_k_N, z_k) / tau) / d2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    norm_dot_product = out_left @ out_right.T / (torch.linalg.norm(out_left, dim=1, keepdim=True) * torch.linalg.norm(out_right, dim=1))
    pos_pairs = torch.diag(norm_dot_product).unsqueeze(1)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = out @ out.T / (torch.linalg.norm(out, dim=1, keepdim=True) * torch.linalg.norm(out, dim=1))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pos = sim_matrix[range(N), range(N, 2*N)]
    ...
    numerator = torch.exp(sim_pos / tau).unsqueeze(1)
    ...
    loss = 1 / (2 * N) * (- torch.sum(torch.log(numerator / denom[:N])) - torch.sum(torch.log(numerator / denom[N:])))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
